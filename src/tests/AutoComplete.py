...
class AutoCompleteTest(unittest.TestCase):
    ...
    def test_suggest_a_word(self):
        auto_complete = AutoComplete()
        target = auto_complete.suggest_a_word
        test_cases = [
            {
                "name": "default_check_1",
                "input": {
        ...
        for test_case in test_cases:
            result = target(**test_case["input"])
            self.assertEqual(True, isinstance(result[0], str) or result[0] is None)
            self.assertEqual(True, isinstance(result[1], float) or isinstance(result[1], int))
            self.assertEqual(result[0], test_case["expected"][0])
            self.assertEqual(True, np.isclose(result[1], test_case["expected"][1]))
